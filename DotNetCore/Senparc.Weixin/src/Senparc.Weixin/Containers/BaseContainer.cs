		...
		protected static IContainerItemCollection ItemCollection
		{
			get
			{
				...
				if (!CollectionList.ContainsKey(cacheKey))
				{
					...
					//保存到缓存列队，等待执行
					LSWMessageQueue mq = new LSWMessageQueue();
					var mqKey = LSWMessageQueue.GenerateKey("ContainerItemCollection", typeof(BaseContainer<TBag>), cacheKey, "InsertItemCollection");
					mq.Add(mqKey, () =>
					{
