...
{
	public class LSWMessageQueue
	{
		private static Dictionary<string, LSWMessageQueueItem> MessageQueueDictionary = new Dictionary<string, LSWMessageQueueItem>(StringComparer.OrdinalIgnoreCase);
		...
		/// <summary>
		/// 获取LSWMessageQueueItem
		/// </summary>
		/// <param name="key"></param>
		/// <returns></returns>
		public LSWMessageQueueItem GetItem(string key)
		{
			lock (MessageQueueSyncLock)
			{
		...
		public LSWMessageQueueItem Add(string key, Action action)
		{
			lock (MessageQueueSyncLock)
			{
				var mqItem = new LSWMessageQueueItem(key, action);
