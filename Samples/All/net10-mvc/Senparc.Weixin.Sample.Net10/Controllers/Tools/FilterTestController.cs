...
namespace Senparc.Weixin.Sample.Net8.Controllers
{
    ...
    public class FilterTestController : Controller
    {
        ...
        {
            return Content("访问正常。当前地址：" + Request.PathAndQuery() + "<br />请点击右上角转发按钮，使用【在浏览器中打开】功能进行测试！<br />或者也可以直接在外部浏览器打开 https://sdk.weixin.senparc.com/FilterTest/ 进行测试。");
        }
        ...
        {
            return Content("访问正常。当前地址：" + Request.PathAndQuery() + "<br />请点击右上角转发按钮，使用【在浏览器中打开】功能进行测试！<br />或者也可以直接在外部浏览器打开 https://sdk.weixin.senparc.com/FilterTest/Redirect 进行测试。");
        }
