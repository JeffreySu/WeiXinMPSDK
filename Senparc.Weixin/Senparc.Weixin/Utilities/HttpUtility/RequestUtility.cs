...
namespace Senparc.Weixin.HttpUtility
{
    public static class RequestUtility
    {
        ...
        public static string HttpPost(string url, CookieContainer cookieContainer = null, Stream postStream = null,byte[] media=null ,string filename =null ,string refererUrl = null, Encoding encoding = null, int timeOut = Config.TIME_OUT, bool checkValidationResult = false)
        {
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "POST";
            request.Timeout = timeOut;
            ...
            #region 处理Form表单文件上传
            var formUploadFile = media != null && media .Length> 0;//是否用Form上传文件
            if (formUploadFile)
            {
                //通过表单上传文件
                postStream = postStream ?? new MemoryStream();
                 ...
                 try
                    {
                        ...
                        //准备文件流
                        using (var fileStream =  new MemoryStream(media))
                        {
                            string formdata = null;
                            if (fileStream != null)
                            {
                                //存在文件
                                formdata = string.Format(fileFormdataTemplate, filename, Path.GetFileName(fileName));
                            }
