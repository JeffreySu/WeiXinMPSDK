...
/*----------------------------------------------------------------
    ...
    文件名：PayBusifavorReceiptsReturnJson.cs
    文件功能描述：营销补差付款返回Json类
...
namespace Senparc.Weixin.TenPayV3.Apis.Marketing
{
    ...
    /// </summary>
    public class PayBusifavorReceiptsReturnJson : ReturnJsonBase
    {
        /// <summary>
        /// 含参构造函数
        /// </summary>
        ...
        /// <param name="status">补差付款单据状态 <para>补差付款单据状态ACCEPTED：受理成功SUCCESS：补差补款成功FAIL：补差付款失败RETURNING：补差回退中PARTIAL_RETURN：补差部分回退FULL_RETURN：补差全额回退</para><para>示例值：SUCCESS</para></param>
        /// <param name="fail_reason">补差付款失败原因 <para>仅在补差付款失败时，返回告知对应失败的原因INSUFFICIENT_BALANCE：扣款商户余额不足NOT_INCOMESPLIT_ORDER：非分账订单EXCEED_SUBSIDY_AMOUNT_QUOTA：超出订单补差总额限制EXCEED_SUBSIDY_COUNT_QUOTA：超出订单补差总数限制OTHER：其他原因</para><para>示例值：INSUFFICIENT_BALANCE</para><para>可为null</para></param>
        ...
        public PayBusifavorReceiptsReturnJson(string subsidy_receipt_id, string stock_id, string coupon_code, string transaction_id, string payer_merchant, string payee_merchant, int amount, string description, string status, string fail_reason, string success_time, string out_subsidy_no, string create_time)
        {
        ...
        /// 无参构造函数
        /// </summary>
        public PayBusifavorReceiptsReturnJson()
        {
        }
