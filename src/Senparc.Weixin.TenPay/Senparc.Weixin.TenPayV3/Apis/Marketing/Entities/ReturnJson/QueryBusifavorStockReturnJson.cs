...
namespace Senparc.Weixin.TenPayV3.Apis.Marketing
{
    /// <summary>
    /// 查询商家券详情返回Json类
    /// <para>详细请参考微信支付官方文档 https://pay.weixin.qq.com/wiki/doc/apiv3/apis/chapter9_2_2.shtml </para>
    /// </summary>
    public class QueryBusifavorStockReturnJson : ReturnJsonBase
    {
        /// <summary>
        /// 含参构造函数
        /// </summary>
        ...
        /// 无参构造函数
        /// </summary>
        public QueryBusifavorStockReturnJson()
        {
        }
        ...
        /// <summary>
        /// 适用商品范围 
        /// <para>用来描述批次在哪些商品可用，会显示在微信卡包中。字数上限为15个，一个中文汉字/英文字母/数字均占用一个字数。 </para>
        ...
        /// <para>批次状态 </para>
        /// <para></para>
        /// <para>UNAUDIT：审核中</para>
        /// <para>RUNNING：运行中</para>
        /// <para>STOPED：已停止 （暂未开放） </para>
        /// <para>PAUSED：暂停发放 （暂未开放） </para>
        /// <para></para>
        ...
        /// <summary>
        /// 券code模式 
        /// <para>枚举值： WECHATPAY_MODE：系统分配券code。 MERCHANT_API：商户发放时接口指定券code。 MERCHANT_UPLOAD：商户上传自定义code，发券时系统随机选取上传的券code。 </para>
        ...
        /// <summary>
        /// 券code数量
        /// <para>当且仅当coupon_code_mode（券code模式）为MERCHANT_UPLOAD（商户上传自定义code）模式时，返回该字段，返回内容为商户上传code的数量信息。 </para>
        /// <para>可为null</para>
        /// </summary>
        public Coupon_Code_Count coupon_code_count { get; set; }

        /// <summary>
        /// 事件通知配置
        /// <para>事件回调通知商户的配置。</para>
        /// <para>可为null</para>
        /// </summary>
        ...
        /// <para>可为null</para>
        /// </summary>
        public Send_Count_Information send_count_information { get; set; }
        ...
        public class Stock_Send_Rule
        {
            ...
            /// 含参构造函数
            /// </summary>
            /// <param name="max_amount">批次总预算  <para>总预算金额，单位：分。</para><para>特殊规则：取值范围1≤value≤100000000000</para><para>示例值：100000</para></param>
            /// <param name="max_coupons">批次最大发放个数  <para>当stock_type为DISCOUNT或EXCHANGE类型时，必须填写。</para><para>特殊规则：取值范围1≤value≤1000000000</para><para>示例值：100</para></param>
            /// <param name="max_coupons_per_user">用户最大可领个数  <para>用户可领个数。</para><para>示例值：5</para></param>
            /// <param name="max_amount_by_day">单天发放上限金额  <para>单天发放上限金额，单位：分（stock_type为NORMAL时可传入此字段控制单天发放上限）。</para><para>特殊规则：取值范围1≤value≤10000000000</para><para>示例值：1000</para><para>可为null</para></param>
            /// <param name="max_coupons_by_day">单天发放上限个数  <para>单天发放上限个数（stock_type为DISCOUNT或EXCHANGE时可传入此字段控制单天发放上限），每个用户最多100张券 。</para><para>特殊规则：取值范围1≤value≤1000000000</para><para>示例值：100</para><para>可为null</para></param>
            ...
            public Stock_Send_Rule(long max_amount, int max_coupons, int max_coupons_per_user, int max_amount_by_day, int max_coupons_by_day, bool natural_person_limit, bool prevent_api_abuse, bool transferable, bool shareable)
            {
            ...
            /// <summary>
            /// 批次总预算 
            /// <para>总预算金额，单位：分。 </para>
            /// <para>特殊规则：取值范围 1 ≤ value ≤ 100000000000</para>
            ...
            /// <summary>
            /// 批次最大发放个数 
            /// <para>当stock_type为DISCOUNT或EXCHANGE类型时，必须填写。 </para>
            ...
            /// <summary>
            /// 单天发放上限金额 
            /// <para>单天发放上限金额，单位：分（stock_type为NORMAL时可传入此字段控制单天发放上限）。 </para>
            /// <para>特殊规则：取值范围 1 ≤ value ≤ 10000000000</para>
            /// <para>示例值：1000 </para>
            /// <para>可为null</para>
            /// </summary>
            ...
            /// <summary>
            /// 单天发放上限个数 
            /// <para>单天发放上限个数（stock_type为DISCOUNT或EXCHANGE时可传入此字段控制单天发放上限），每个用户最多100张券 。 </para>
            /// <para>特殊规则：取值范围 1 ≤ value ≤ 1000000000</para>
            /// <para>示例值：100 </para>
            /// <para>可为null</para>
            /// </summary>
        ...
        public class Custom_Entrance
        {
            ...
            /// <param name="appid">商户公众号appid  <para>可配置商户公众号，从券详情可跳转至公众号，用户自定义字段。</para><para>注：若创建商家券接口未填写该参数，该参数默认返回微信支付的appid，用户点击进入微信支付公众号</para><para>示例值：wx324345hgfhfghfg</para><para>可为null</para></param>
            ...
            /// <para>注：若创建商家券接口未填写该参数，该参数默认返回微信支付的appid，用户点击进入微信支付公众号 </para>
        ...
        public class Display_Pattern_Info
        {
            ...
            /// 含参构造函数
            /// </summary>
            /// <param name="description">使用须知  <para>用于说明详细的活动规则，会展示在代金券详情页。</para><para>示例值：深圳南山区门店可用</para><para>可为null</para></param>
            /// <param name="merchant_logo_url">商户logo  <para>商户logo的URL地址，可通过《图片上传API》获得图片URL地址。1、商户logo大小需为120像素*120像素。2、支持JPG/JPEG/PNG格式，且图片小于1M。</para><para>示例值：https://qpic.cn/xxx</para><para>可为null</para></param>
        ...
        public class Coupon_Code_Count
        {

            /// <summary>
            /// 含参构造函数
            /// </summary>
            /// <param name="total_count">该批次总共已上传的code总数  <para>该批次总共已上传的code总数。</para><para>示例值：100</para></param>
            /// <param name="available_count">该批次当前可用的code数  <para>该批次当前可用的code数。</para><para>注：该批次未发放的code数</para><para>示例值：50</para></param>
            public Coupon_Code_Count(ulong total_count, ulong available_count)
            {
            ...
            /// 无参构造函数
            /// </summary>
            public Coupon_Code_Count()
            {
            }

            /// <summary>
            /// 该批次总共已上传的code总数 
            /// <para>该批次总共已上传的code总数。 </para>
            /// <para>示例值：100 </para>
            /// </summary>
            ...
            /// <summary>
            /// 该批次当前可用的code数 
            /// <para>该批次当前可用的code数。 </para>
            /// <para>注：该批次未发放的code数</para>
            /// <para>示例值：50 </para>
            /// </summary>
            public ulong available_count { get; set; }
        ...
        public class Send_Count_Information
        {

            /// <summary>
            /// 含参构造函数
            /// </summary>
            /// <param name="total_send_num">已发放券张数  <para>批次已发放的券数量，满减、折扣、换购类型会返回该字段</para><para>示例值：1</para><para>可为null</para></param>
            /// <param name="total_send_amount">已发放券金额  <para>批次已发放的预算金额，满减券类型会返回该字段</para><para>示例值：34</para><para>可为null</para></param>
            /// <param name="today_send_num">单天已发放券张数  <para>批次当天已发放的券数量，设置了单天发放上限的满减、折扣、换购类型返回该字段</para><para>示例值：1</para><para>可为null</para></param>
            /// <param name="today_send_amount">单天已发放券金额  <para>批次当天已发放的预算金额，设置了当天发放上限的满减券类型返回该字段</para><para>示例值：34</para><para>可为null</para></param>
            public Send_Count_Information(ulong total_send_num, ulong total_send_amount, ulong today_send_num, ulong today_send_amount)
            {
                this.total_send_num = total_send_num;
                this.total_send_amount = total_send_amount;
                this.today_send_num = today_send_num;
                this.today_send_amount = today_send_amount;
            }

            /// <summary>
            /// 无参构造函数
            /// </summary>
            public Send_Count_Information()
            {
            }

            /// <summary>
            /// 已发放券张数 
            /// <para>批次已发放的券数量，满减、折扣、换购类型会返回该字段 </para>
            /// <para>示例值：1 </para>
            /// <para>可为null</para>
            /// </summary>
            public ulong total_send_num { get; set; }

            /// <summary>
            /// 已发放券金额 
            /// <para>批次已发放的预算金额，满减券类型会返回该字段 </para>
            /// <para>示例值：34 </para>
            /// <para>可为null</para>
            /// </summary>
            public ulong total_send_amount { get; set; }

            /// <summary>
            /// 单天已发放券张数 
            /// <para>批次当天已发放的券数量，设置了单天发放上限的满减、折扣、换购类型返回该字段 </para>
            /// <para>示例值：1 </para>
            /// <para>可为null</para>
            /// </summary>
            public ulong today_send_num { get; set; }

            /// <summary>
            /// 单天已发放券金额 
            /// <para>批次当天已发放的预算金额，设置了当天发放上限的满减券类型返回该字段 </para>
            /// <para>示例值：34 </para>
            /// <para>可为null</para>
            /// </summary>
            public ulong today_send_amount { get; set; }
