...
/*----------------------------------------------------------------
    ...
    文件名：QueryProfitsharingReturnJson.cs
    文件功能描述：查询分账结果返回Json类
...
namespace Senparc.Weixin.TenPayV3.Apis.Profitsharing
{
    ...
    /// </summary>
    public class QueryProfitsharingReturnJson : ReturnJsonBase
    {

        /// <summary>
        /// 无参构造函数
        /// </summary>
        public QueryProfitsharingReturnJson()
        {
        }
        ...
        public QueryProfitsharingReturnJson(string transaction_id, string out_order_no, string order_id, string state, Receivers[] receivers)
        {
        ...
        /// <param name="state">分账单状态 <para>分账单状态（每个接收方的分账结果请查看receivers中的result字段），枚举值：1、PROCESSING：处理中2、FINISHED：分账完成</para><para>示例值：FINISHED</para></param>
        /// <param name="receivers">分账接收方列表 <para>分账接收方列表</para><para>可为null</para></param>
        public QueryProfitsharingReturnJson(string sub_mchid, string transaction_id, string out_order_no, string order_id, string state, Receivers[] receivers)
        {
        ...
        public class Receivers
        {
            ...
            public Receivers(int amount, string description, string type, string account, string result, string fail_reason, string detail_id, string create_time, string finish_time)
            {
                this.amount = amount;
