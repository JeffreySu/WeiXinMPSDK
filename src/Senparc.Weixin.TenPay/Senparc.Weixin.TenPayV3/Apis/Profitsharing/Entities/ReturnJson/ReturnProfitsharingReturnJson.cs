...
/*----------------------------------------------------------------
    ...
    文件名：ReturnProfitsharingReturnJson.cs
    文件功能描述：请求分账回退返回Json类
...
namespace Senparc.Weixin.TenPayV3.Apis.Profitsharing
{
    ...
    /// </summary>
    public class ReturnProfitsharingReturnJson : ReturnJsonBase
    {

        /// <summary>
        /// 无参构造函数
        /// </summary>
        public ReturnProfitsharingReturnJson()
        {
        }
        ...
        /// <param name="out_order_no">商户分账单号 <para>商户系统内部的分账单号，在商户系统内部唯一，同一分账单号多次请求等同一次。取值范围：[0-9a-zA-Z_*@-]</para><para>示例值：P20150806125346</para></param>
        /// <param name="out_return_no">商户回退单号 <para>此回退单号是商户在自己后台生成的一个新的回退单号，在商户后台唯一</para><para>示例值：R20190516001</para></param>
        ...
        public ReturnProfitsharingReturnJson(string order_id, string out_order_no, string out_return_no, string return_id, string return_mchid, int amount, string description, string result, string fail_reason, string create_time, string finish_time)
        {
        ...
        /// <param name="create_time">创建时间 <para>分账回退创建时间，遵循rfc3339标准格式，格式为YYYY-MM-DDTHH:mm:ss.sss+TIMEZONE，YYYY-MM-DD表示年月日，T出现在字符串中，表示time元素的开头，HH:mm:ss.sss表示时分秒毫秒，TIMEZONE表示时区（+08:00表示东八区时间，领先UTC8小时，即北京时间）。例如：2015-05-20T13:29:35.120+08:00表示，北京时间2015年5月20日13点29分35秒。</para><para>示例值：2015-05-20T13:29:35.120+08:00</para></param>
        /// <param name="finish_time">完成时间 <para>分账回退完成时间，遵循rfc3339标准格式，格式为YYYY-MM-DDTHH:mm:ss.sss+TIMEZONE，YYYY-MM-DD表示年月日，T出现在字符串中，表示time元素的开头，HH:mm:ss.sss表示时分秒毫秒，TIMEZONE表示时区（+08:00表示东八区时间，领先UTC8小时，即北京时间）。例如：2015-05-20T13:29:35.120+08:00表示，北京时间2015年5月20日13点29分35秒。</para><para>示例值：2015-05-20T13:29:35.120+08:00</para></param>
        public ReturnProfitsharingReturnJson(string sub_mchid, string order_id, string out_order_no, string out_return_no, string return_id, string return_mchid, int amount, string description, string result, string fail_reason, string create_time, string finish_time)
        {
        ...
        /// <summary>
        /// 商户分账单号
        /// <para>商户系统内部的分账单号，在商户系统内部唯一，同一分账单号多次请求等同一次。 取值范围：[0-9a-zA-Z_*@-]</para>
        ...
        /// <summary>
        /// 商户回退单号
        /// <para>此回退单号是商户在自己后台生成的一个新的回退单号，在商户后台唯一</para>
