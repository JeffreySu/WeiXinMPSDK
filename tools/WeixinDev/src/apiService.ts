...
export class WeixinApiService {
    private readonly defaultApiUrl = 'https://sdk.weixin.senparc.com/AiDoc';
     ...
     * @returns 解析结果
     */
    public async queryInterface(query: string): Promise<ApiQueryResult> {
        try {
     ...
     * 重试查询（用于处理动态内容）
     */
    private async queryInterfaceWithRetry(query: string): Promise<ApiQueryResult> {
        try {
     ...
     * 解析API响应HTML
     */
    private parseApiResponse(html: string): ApiQueryResult {
        try {
     ...
     * 清理注释文本
     */
    private cleanCommentsText(text: string): string {
        return text
            .replace(/\s+/g, ' ')
            .replace(/^\s+|\s+$/g, '')
            .split('\n')
     ...
     * 生成备用结果（当API无法正常解析时）
     */
    private generateFallbackResult(query: string): ApiQueryResult {
        // 根据查询内容生成基础的代码模板
