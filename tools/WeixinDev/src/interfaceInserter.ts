...
import { WeixinApiService, ApiQueryResult } from './apiService';

export class WeixinInterfaceInserter {
    constructor(private readonly _apiService: WeixinApiService) {}

    /**
     * 显示输入对话框（右键菜单调用）
     */
    public async showInputDialog(): Promise<void> {
     ...
     * 插入代码到编辑器
     */
    public async insertCodeToEditor(code: string, comments?: string): Promise<void> {
     ...
     * 格式化代码
     */
    private formatCode(code: string, editor: vscode.TextEditor, position: vscode.Position): string {
        // 获取当前行的缩进
     ...
     * 格式化插入的代码
     */
    private async formatInsertedCode(editor: vscode.TextEditor, insertPosition: vscode.Position, insertedText: string): Promise<void> {
        try {
            // 计算插入内容的范围
     ...
     * 插入代码片段（支持占位符）
     */
    public async insertCodeSnippet(code: string, comments?: string): Promise<void> {
        const editor = vscode.window.activeTextEditor;
     ...
     * 处理代码中的占位符
     */
    private processCodePlaceholders(code: string): string {
        // 替换常见的占位符
        let processed = code
